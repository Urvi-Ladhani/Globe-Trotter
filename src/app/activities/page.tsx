...
export default async function ActivitiesPage({
  ...
  // 1. Fetch approved activities with optional city & category filters
  ...
  // 3. Fetch user's active trips & stops for "Add to Trip" dropdown
  ...
  const { data: userStops } = tripIds.length
    ? await supabase.from("trip_stops").select("stop_id, trip_id, city_id").in("trip_id", tripIds)
    : { data: [] };

  const stopMap = new Map((userStops ?? []).map((s) => [s.stop_id, s]));
    ...
    <div className="flex min-h-screen flex-col bg-[#FDFBF7]">
      ...
      <div className="border-b border-teal-900/10 bg-gradient-to-b from-[#E0F2FE]/40 to-transparent py-8 px-4">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-[#0891B2]">Activity Catalog</span>
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-4xl">Experiences & Things To Do</h1>
            <p className="mt-1 text-sm text-slate-600">Discover curated food tours, sightseeing spots, outdoor adventures, and more.</p>
          </div>
          <a
            ...
            return (
              ...
              >
                <div className="p-5">
                  ...
                  <p className="text-xs text-slate-500 font-medium">📍 {city?.name ?? "City"}, {city?.country}</p>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3 text-xs">
                    ...
                    {act.duration_minutes ? (
                      <span className="text-slate-500 font-medium">⏱ {act.duration_minutes} mins</span>
                    ) : null}
                  </div>
                </div>
                    ...
                    <form action={addActivityToTripStop} className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <input
                          ...
                          title="Day #"
                          placeholder="Day"
                          className="w-14 pacific-input text-xs py-1.5"
                        />
                        <button
                          type="submit"
                          className="btn-coral px-3 py-1.5 text-xs font-bold shadow-xs"
                        >
                          + Add
                        </button>
                      </div>
        ...
        <section id="submit-activity" className="pacific-card p-8">
          ...
          <form action={submitActivity} className="mt-6 grid gap-4 sm:grid-cols-2 text-xs font-semibold text-slate-700">
            ...
            <label className="flex flex-col gap-1 sm:col-span-2">
              Description
              <textarea name="description" rows={3} placeholder="Highlights, tips, booking advice..." className="pacific-input text-xs font-normal" />
            </label>

            <div className="sm:col-span-2">
              <button
                type="submit"
                className="btn-coral px-6 py-3 text-xs font-bold shadow-md"
              >
